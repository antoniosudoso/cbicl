...
This repository contains the implementation of **CBICL-BB** and **CBICL-LR**, described in the paper ["Exact and Heuristic Algorithms for Constrained Biclustering"](https://link.springer.com/article/10.1007/s10589-026-00764-6). **CBICL-BB** is an exact algorithm, based on the branch-and-cut technique, for constrained biclustering through the $k$-densest-disjoint biclique criterion with must-link and cannot-link constraints. **CBICL-LR** is a heuristic algorithm based on a low-rank factorization of the SDP relaxation.  Datasets and constraint sets used in the experiments are available in the `Data` folder.
