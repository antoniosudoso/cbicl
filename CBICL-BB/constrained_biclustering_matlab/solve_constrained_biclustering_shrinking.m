function [result] = solve_constrained_biclustering_shrinking(W, k, ML_u, CL_u, ML_v, CL_v, ...
    init_B_cell, global_lb, global_Xu, global_Xv, params)

    % SOLVE SDP RELAXATION FOR CONSTRAINED BICLUSTERING
    
    % W: weight matrix (n x m)
    % k: number of clusters
    % ML_u: matrix of must-link constraints between vertices in U (n_ml_U x 2)
    % CL_u: matrix of cannot-link constraints between vertices in U (n_cl_U x 2)
    % ML_v: matrix of must-link constraints between vertices in V (n_ml_V x 2)
    % CL_v: matrix of cannot-link constraints between vertices in V (n_cl_V x 2)
    % init_B_cell: cell array of initial valid inequalities (for empty pass cell(0))
    % global_lb: best known lower bound (double)
    % global_Xu: best known (normalized) assignment matrix (n x k)
    % global_Xv: best known (normalized) assignment matrix (m x k)
    ...
    % params.cp_activeineq: tolerance for checking active inequalities (default 1e-4) 
    ...
    % params.heuristic_iter: number of iterations for the heuristic(defualt 50)
    ...
    [blk, At, b, C, L, T_U, ML_graph_U, T_V, ML_graph_V, B_cell_shr, l_shr] = build_constrained_biclustering_shrinking(...
        W, k, ML_u, CL_u, ML_v, CL_v, init_B_cell);
    ...
    %[lb, Xu, Xv, ilptime] = constrained_biclustering_heuristic(Z, W, k, ML_u, CL_u, ML_v, CL_v, params.gurobi_verbose);
    [lb, Xu, Xv, ilptime] = constrained_biclustering_heuristic_shrinking(Zshr, T_U, T_V, W, k, CL_u, CL_v, ...
        params.gurobi_verbose, params.heuristic_iter);
    ...
    result.htime_list = [result.htime_list; ilptime];
    ...
    %  3 - upper bound increase is not sufficiently large
    ...
    for i=1:params.cp_maxiter
        ...
        if ~isempty(B_cell) && ~isempty(l)
            ...
            B_cell_or = B_cell_or(active_idx); % remove inactive from the pool of cuts to inherit
        ...
        B_ineq = shrink_cuts_new(ML_graph_U, ML_graph_V, B_ineq_or, n, m, original_n);
