...
SDPResult solve_sdp(std::unique_ptr<matlab::engine::MATLABEngine> &matlabPtr, matlab::data::ArrayFactory &factory, arma::mat &W, int k,
    ...
    // matlab::data::TypedArray<double> matlab_ml_UV = vector_pair_to_matlab_matrix(factory, global_ml_pairs_UV);
    // matlab::data::TypedArray<double> matlab_cl_UV = vector_pair_to_matlab_matrix(factory, global_cl_pairs_UV);

    // Create StructArray
    std::vector<std::string> f = {"n_threads", "bb_tol", "sdp_verbose", "sdp_tol", "cp_maxiter", "cp_tol", "cp_maxineq",
                                  "cp_percineq", "cp_epsineq", "cp_activeineq", "gurobi_verbose", "heuristic_iter"};
    ...
    struct_matlab[0]["heuristic_iter"] = factory.createScalar<int>(heuristic_iter);

    std::vector<matlab::data::Array> args({matlab_W, matlab_k, matlab_ml_U, matlab_cl_U, matlab_ml_V, matlab_cl_V, struct_matlab});

    // Call MATLAB function and return result
    const size_t n_return = 1;
    matlabPtr->eval(u"clear");
    std::vector<matlab::data::Array> result = matlabPtr->feval(u"call_solve_constrained_biclustering_root",n_return, args);
    ...
    matlab::data::TypedArray<double> field_htime = structArray[0]["htime"];

    // matlab::data::TypedArray<double> field_implied_ml_U = structArray[0]["implied_ML_u"];
    // matlab::data::TypedArray<double> filed_implied_ml_V = structArray[0]["implied_ML_v"];
    // add implied ML constraints to the global pairs of ML constraints
    // matlab_matrix_to_vector_pair(field_implied_ml_U, global_ml_pairs_U);
    // matlab_matrix_to_vector_pair(filed_implied_ml_V, global_ml_pairs_V);
    ...
    int n = (int) field_n[0];
    int m = (int) field_m[0];
    double htime = field_htime[0];

    return SDPResult{n, m, lb, ub, Xu_assignment, Xv_assignment, n_ineq, cp_iter, cp_flag, branching_type, i_idx, j_idx, htime, B_vector};
}
...
SDPResult solve_sdp(std::unique_ptr<matlab::engine::MATLABEngine> &matlabPtr, matlab::data::ArrayFactory &factory,
    ...
    const size_t n_return = 1;
    matlabPtr->eval(u"clear");
    std::vector<matlab::data::Array> result = matlabPtr->feval(u"call_solve_constrained_biclustering_child",n_return, args);
...
std::pair<JobData *, JobData *> build_child_problem(int job_type, NodeData *node_data, InputData *input_data, SharedData  *shared_data) {
    ...
    // inherit ML_uv and CL_uv
    // child_node->global_ml_pairs_UV = parent->global_ml_pairs_UV;
    // child_node->global_cl_pairs_UV = parent->global_cl_pairs_UV;
...

    {
        ...
        print_log_sdp(n, m,parent->id, child_node->id, parent->ub, child_node->ub, time, cp_iter, cp_flag, n_ineq,
                      child_node->lb, htime, shared_data->global_lb,branching_type, node_data->i, node_data->j, node_gap, shared_data->gap, open, ub_updated);
...
std::pair<JobData *, JobData *> build_root_problem(MatlabStruct *matlab_struct, InputData *input_data, SharedData *shared_data) {
    ...
    // add constraints at the root node
    root->global_ml_pairs_U = input_data->constraints.ml_pairs_U;
    root->global_cl_pairs_U = input_data->constraints.cl_pairs_U;
    root->global_ml_pairs_V = input_data->constraints.ml_pairs_V;
    root->global_cl_pairs_V = input_data->constraints.cl_pairs_V;

    auto start_time = std::chrono::high_resolution_clock::now();

    SDPResult sdp_result = solve_sdp(matlab_struct->matlabPtr, matlab_struct->factory, input_data->W, input_data->k,
                                     root->global_ml_pairs_U, root->global_cl_pairs_U,
                                     root->global_ml_pairs_V, root->global_cl_pairs_V);
    ...
    print_log_sdp(n, m,-1, root->id, std::numeric_limits<double>::infinity(),
                  root->ub, time, cp_iter, cp_flag, n_ineq, root->lb, htime, shared_data->global_lb,
...
void sdp_branch_and_bound(arma::mat &W, int k, UserConstraints &constraints) {
    ...
    input_data->constraints = constraints; // background knowledge
    ...
    std::string Xu_str = "Xu";
    save_X_to_file(result_Xu, Xu_str);
    std::string Xv_str = "Xv";
    save_X_to_file(result_Xv, Xv_str);
