...
void matlab_matrix_to_vector_pair(matlab::data::TypedArray<double> &M, std::vector<std::pair<int, int>> &pairs) {

    const size_t n_pairs = M.getDimensions()[0];
    for (size_t i = 0; i < n_pairs; i++) {
        int i_pair = (int) M[i][0];
        int j_pair = (int) M[i][1];
        pairs.emplace_back(i_pair, j_pair);
    }
